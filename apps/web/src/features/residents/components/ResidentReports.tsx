...
import { Camera, ChevronDown, ChevronLeft, ChevronRight } from "lucide-react";

type ReportTab = "new" | "history";
    ...
    <main className="min-h-0 min-w-0 flex-1 overflow-hidden bg-surface p-5">
      <div className="mx-auto flex h-full w-full max-w-350 flex-col">
        ...
        <div key={activeTab} className="report-slide-in">
            {activeTab === "new" ? <NewReport /> : <ReportHistory />}
        </div>
        </section>
      </div>
    </main>
...
function NewReport(): React.ReactNode {
  return (
...
function ReportHistory(): React.ReactNode {
  return (
    <div className="px-6 pb-6 pt-5">
      <div className="space-y-3">
        {reportHistory.map((report) => (
          <div
            key={report.id}
            className={`flex min-h-20 items-center justify-between rounded-md border border-gray-200 bg-white px-5 ${report.borderClass}`}
          >
            <div className="min-w-0">
              <p className="text-[7px] text-gray-400">{report.date}</p>
            ...
            <div className="flex shrink-0 flex-col items-end gap-1">
              <span
                className={`rounded-full px-2 py-0.5 text-[7px] font-bold ${report.statusClass}`}
              >
                {report.status}
              </span>
