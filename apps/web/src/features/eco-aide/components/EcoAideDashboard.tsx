...
export default function EcoAideDashboard(): React.ReactNode {
  const navigate = useNavigate();
  const [activeTracker, setActiveTracker] = useState<any | null>(null);
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      try {
        try {
          const trackerRes = await apiClient.get<{ success: boolean; data: any }>("/trucks/me");
          if (trackerRes.data.success) {
            setActiveTracker(trackerRes.data.data);
          }
        } catch (error: any) {
        ...
        console.error("Failed to load Eco-Aide dashboard data", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const isRouteActive = activeTracker?.status === "active";
  const plannedStops = activeTracker?.plannedRoute || [];
  ...
  const stats = [
    ...
    },
    {
      ...
      subColor: isRouteActive ? "text-orange-660 bg-orange-50" : "text-emerald-600 bg-emerald-50",
      icon: CheckCircle2,
    },
    {
      label: "Vehicle Unit",
      value: activeTracker?.plateNumber || "No Vehicle",
      sub: activeTracker ? "ASSIGNED" : "UNASSIGNED",
      subColor: activeTracker ? "text-emerald-600 bg-emerald-50" : "text-red-650 bg-red-50",
      icon: Truck,
    },
  ];
            ...
            <div className="space-y-5 lg:col-span-3">
              {isRouteActive ? (
                ...
                <Card className="overflow-hidden rounded-2xl border-0 bg-white dark:bg-slate-900 dark:border-slate-800 p-6 text-center shadow-sm lg:p-8">
                  <CardContent className="flex flex-col items-center justify-center p-0 py-6">
                    <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-emerald-50 dark:bg-slate-850 text-emerald-600 dark:text-emerald-400">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <Card className="flex items-center gap-4 rounded-xl border border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-sm">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-50 dark:bg-slate-850 text-emerald-600 dark:text-emerald-400">
                  ...
                  <div>
                    ...
                    <p className="text-sm leading-none font-bold text-gray-900 dark:text-white">
                      {activeTracker ? "LGU Assigned Route" : "No Assignment"}
                    </p>
                  </div>
                </Card>
                <Card className="flex items-center gap-4 rounded-xl border border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-sm">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-50 dark:bg-slate-850 text-blue-600 dark:text-blue-450">
                  ...
                  <div>
                    ...
                    <p className="text-sm leading-none font-bold text-gray-900 dark:text-white">
                      {activeTracker ? "Operational" : "Not Assigned"}
                    </p>
                  </div>
                </Card>
              </div>
              ...
              <Card className="flex h-full flex-col overflow-hidden rounded-2xl border border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
                <div className="flex items-center justify-between border-b border-gray-50 dark:border-slate-850 p-5">
                  <h3 className="text-[10px] font-bold tracking-widest text-gray-900 dark:text-white uppercase">
                    {isRouteActive ? "Active Route Progress" : "Recent Route Runs"}
                  </h3>
                  {isRouteActive && (
                ...
                <CardContent className="space-y-5 p-5">
                  {isRouteActive ? (
                    <>
                      ...
                      <div className="space-y-1.5">
                        <div className="relative h-1.5 w-full overflow-hidden rounded-full bg-gray-100 dark:bg-slate-800">
                          <div
                            className="absolute top-0 left-0 h-full bg-[#0f2419] dark:bg-emerald-500 transition-all duration-300"
                      ...
                      <div className="max-h-[300px] space-y-3 overflow-y-auto pr-1">
                        {plannedStops.slice(0, 5).map((item: any, idx: number) => (
                          <div
                            key={item.id || idx}
                            ...
                            <div className="flex items-start justify-between">
                              <div className="space-y-0.5">
                                ...
                                <p className="text-[10px] leading-tight font-medium text-gray-400">
                                  {item.detail || "Residential Pickup"}
                                </p>
                              </div>
                              <Badge
                    ...
                    <div className="max-h-[400px] space-y-3 overflow-y-auto pr-1">
                      {completedRoutes.slice(0, 5).map((route, idx) => {
                        const totalStops = route.stops?.length || 0;
                        const dateFormatted = format(
                          new Date(route.createdAt),
                          "MMM d, yyyy · h:mm a",
                        );
                        const firstStop = route.stops?.[0]?.name || "Route Run";

                        return (
                          <div
                            key={route.id || idx}
                            className="group relative flex flex-col gap-2 rounded-xl border border-gray-50 dark:border-slate-850 bg-white dark:bg-slate-950 p-4 transition-all hover:bg-gray-50/55"
                          >
                            <div className="flex items-start justify-between">
                              <div className="space-y-0.5">
                                <p className="text-[9px] font-medium tracking-tight text-gray-400 uppercase">
                                ...
                                <h4 className="text-sm font-bold tracking-tight text-gray-900 dark:text-white capitalize">
                                  {firstStop} {totalStops > 1 ? `+ ${totalStops - 1} stops` : ""}
                                </h4>
                              </div>
                              <Badge className="rounded-full border-0 bg-emerald-50 text-emerald-600 px-2 py-0.5 text-[8px] font-bold tracking-widest uppercase">
                                DONE
                              </Badge>
                            </div>
                          </div>
