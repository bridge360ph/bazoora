...
export default function DriverRoute(): React.ReactNode {
  ...
  useEffect(() => {
    ...
    async function getMyTruck() {
      try {
        const res = await fetch(`${env.VITE_API_URL}/trucks/me`, {
        ...
        if (json.success && json.data) {
          ...
          console.log("Current Location:", truck.currentLocation);
          console.log("Planned Route:", truck.plannedRoute);
  ...
  useEffect(() => {
    ...
    autoDriveTimer.current = setInterval(() => {
      ...
      setGpsPos((prev) => {
        ...
        fetch(`${env.VITE_API_URL}/trucks/${truckId}/location`, {
          method: "POST",
          headers: {
  ...
  useEffect(() => {
    ...
    watchIdRef.current = navigator.geolocation.watchPosition(
      async (position) => {
        ...
        try {
          await fetch(`${env.VITE_API_URL}/trucks/${truckId}/location`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  const startCollection = async () => {
    ...
    try {
      const res = await fetch(`${env.VITE_API_URL}/trucks/${truckId}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${useAuthStore.getState().accessToken}`,
  ...
  gpsPos && Number.isFinite(gpsPos[0]) && Number.isFinite(gpsPos[1])
    ? gpsPos
    : [13.8248, 121.3964];

  console.log("GPS POS:", gpsPos);
  ...
  const finishCollection = async () => {
    ...
    setIsStarting(true);
    try {
      await fetch(`${env.VITE_API_URL}/trucks/${truckId}/finish-route`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
  ...
  const syncOfflineQueue = useCallback(async () => {
    ...
    for (const item of queue) {
      try {
        ...
        if (item.base64Photo) {
          const uploadRes = await fetch(`${env.VITE_API_URL}/uploads/image`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${accessToken}`,
        ...
        await fetch(`${env.VITE_API_URL}/trucks/${truckId}`, {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${accessToken}`,
                ...
                <div className="mt-3 flex items-start justify-between rounded-xl bg-gray-50 p-2.5">
                  ...
                  {stop.proofPhotoUrl ? (
                    <div className="h-10 w-10 overflow-hidden rounded-lg border border-gray-200">
                      <img
                        src={`${env.VITE_SOCKET_URL}${stop.proofPhotoUrl}`}
                        alt="Proof"
          ...
          {!isPlanning && (
            <Button
              ...
              onClick={async () => {
                ...
                if (truckId) {
                  fetch(`${env.VITE_API_URL}/trucks/${truckId}`, {
                    method: "PATCH",
                    headers: {
                      "Content-Type": "application/json",
                      Authorization: `Bearer ${accessToken}`,
