...
export function LocationPermissionModal({ onAllow }: LocationPermissionProps = {}) {
  ...
  const handleRetry = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        console.warn("Location successfully enabled live:", position.coords);
        setIsDenied(false);
        setShow(false);
        if (onAllow) onAllow();
  ...
  useEffect(() => {
    ...
    const triggerNativePrompt = () => {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          console.warn("Automatic location check succeeded:", position.coords);
          setShow(false);
          if (onAllow) onAllow();
        },
        (error) => {
    ...
    if (navigator.permissions) {
      navigator.permissions.query({ name: "geolocation" }).then((result) => {
        console.warn("Location permission state:", result.state);
  ...
  console.warn("Permissions API not supported, showing modal by default");
